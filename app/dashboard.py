...
overlay = "rgba(15,23,42,0.4)" if mode == "Dark 🌙" else "rgba(248,250,252,0.4)"
...
st.markdown(f"""
<style>
.stApp {{ background: url('https://images.unsplash.com/photo-1567306226416-28f0efdc88ce?q=80&w=1470') no-repeat center fixed; background-size: cover; }}
...
.header-container {{ padding:20px; background-color:{card_bg}; border-radius:15px; border-left:10px solid {accent_color}; margin-bottom:25px; }}
...
def generate_forecast(hist_series, horizon, scenario, noise_val):
    ...
    for i in range(horizon):
        ...
        # بناء الميزات الدورية (Cyclical Features)
        d_sin = np.sin(2*np.pi*next_date.dayofweek/7)
        d_cos = np.cos(2*np.pi*next_date.dayofweek/7)
        m_sin = np.sin(2*np.pi*next_date.month/12)
        m_cos = np.cos(2*np.pi*next_date.month/12)
        
        # الـ Lags
        l1 = current_hist.iloc[-1]
        l7 = current_hist.iloc[-7] if len(current_hist)>=7 else current_hist.mean()

        # إنشاء قاموس الميزات مع ضمان مطابقة أسماء الأعمدة في موديليك
        feat_dict = {
            'day_sin': d_sin, 'day_cos': d_cos,
            'month_sin': m_sin, 'month_cos': m_cos,
            'lag_1': l1, 'lag_7': l7,
            'lag_1_scaled': l1, 'lag_7_scaled': l7 # سيتم معاملتها كأعمدة إضافية لو الموديل طلبها
        }
        
        # تحويل لـ DataFrame
        X_df = pd.DataFrame([feat_dict])
        
        # التعديل الجوهري: إضافة أي أعمدة ناقصة يطلبها الموديل وترتيبها
        for col in feature_names:
            if col not in X_df.columns:
                X_df[col] = 0.0 # إضافة العمود الناقص بقيمة صفرية
        
        X_df = X_df[feature_names] # فرض الترتيب الصحيح للأعمدة
        ...
        # تحديث السلسلة التاريخية باليوم الجديد (Recursive)
...
# ================== Execution ==================
st.markdown(f'<div class="header-container"><h1 style="color:{accent_color};">Retail AI Pro</h1><p style="color:{text_color};">Eng. Goda Emad | Smart Forecasting System</p></div>', unsafe_allow_html=True)

with st.sidebar:
    st.header("🛒 إعدادات التوقع")
    scenario = st.selectbox("سيناريو السوق", ["واقعي", "متفائل (+15%)", "متشائم (-15%)"])
    horizon = st.slider("عدد الأيام", 7, 30, 14)
    noise_lvl = st.slider("التقلب", 0.0, 0.1, 0.03)
...
if run_btn and model is not None:
    ...
    c1.markdown(f"<div class='metric-box'>المبيعات المتوقعة<br><h2>${preds_sel.sum():,.0f}</h2></div>", unsafe_allow_html=True)
    c2.markdown(f"<div class='metric-box'>المتوسط اليومي<br><h2>${preds_sel.mean():,.0f}</h2></div>", unsafe_allow_html=True)
    c3.markdown(f"<div class='metric-box'>الثقة<br><h2>82%</h2></div>", unsafe_allow_html=True)

    # Chart
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=sales_hist.index[-30:], y=sales_hist.values[-30:], name="آخر 30 يوم", line=dict(color="gray")))
    
    colors = {"واقعي":"#10B981", "متفائل (+15%)":"#F59E0B", "متشائم (-15%)":"#EF4444"}
    for sc, (d, p) in all_preds.items():
        fig.add_trace(go.Scatter(x=d, y=p, name=sc, line=dict(color=colors[sc], width=3)))
    
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color=text_color)
    st.plotly_chart(fig, use_container_width=True)
else:
    st.info("👈 اضغط تشغيل لعرض التوقعات")
