...
# ================== 3️⃣ Forecast Engine & Plotly Charts (Premium Secure Version) ==================
...
# --- 2️⃣ دالة التوقع المحسّنة ---
def generate_forecast(hist, horizon, scenario_multiplier, residual_std):
    if hist.empty or hist['sales'].sum() == 0:
        # حالة البيانات غير كافية
        dates = pd.date_range(pd.Timestamp.now(), periods=horizon)
        return [0]*horizon, [0]*horizon, [0]*horizon, dates

    np.random.seed(42)
    temp_sales = hist['sales'].tail(30).fillna(0).tolist()
    start_date = pd.Timestamp.now().normalize()
    logical_cap = max(hist['sales'].max()*5, 1e5)
    actual_std = hist['sales'].std()
    safe_std = residual_std if 0 < residual_std < (actual_std*3) else (actual_std if actual_std>0 else 500)

    preds, lows, ups, forecast_dates = [], [], [], []

    for i in range(horizon):
        date_i = start_date + pd.Timedelta(days=i+1)
        forecast_dates.append(date_i)

        feats = {
            'dayofweek_sin': np.sin(2*np.pi*date_i.dayofweek/7),
            'dayofweek_cos': np.cos(2*np.pi*date_i.dayofweek/7),
            'month_sin': np.sin(2*np.pi*(date_i.month-1)/12),
            'month_cos': np.cos(2*np.pi*(date_i.month-1)/12),
            'lag_1': temp_sales[-1],
            'lag_7': temp_sales[-7] if len(temp_sales)>=7 else np.mean(temp_sales),
            'rolling_mean_7': np.mean(temp_sales[-7:]),
            'rolling_mean_14': np.mean(temp_sales[-14:]),
            'is_weekend': int(date_i.dayofweek>=5),
            'was_closed_yesterday': int(temp_sales[-1]<=0)
        }
        ...
        p = np.expm1(np.clip(p_log, 0, 12)) * scenario_multiplier

        # Guardrail: النمو اليومي + الحد الأعلى
        max_daily_growth = temp_sales[-1]*1.15
        p = min(p, max_daily_growth, logical_cap)
        temp_sales.append(p)

        # Confidence Interval
        boost = 1.96 * safe_std * np.sqrt(i+1)
        preds.append(p)
        lows.append(max(0, p-boost))
        ups.append(min(p+boost, logical_cap*1.2))
